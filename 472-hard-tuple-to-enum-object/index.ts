...
type Enum<T extends readonly string[], N extends boolean = false, Count extends unknown[] = [], R extends Record<string, unknown> = {}> = T extends readonly [infer First, ...infer Rest]
? Rest extends string[]
  ? Enum<Rest, N, [...Count, unknown], {
    readonly [Key in ((keyof R) | Capitalize<First & string>)]:
      N extends true
      ? Key extends keyof R ? R[Key] : Count['length']
      : Key extends keyof R ? R[Key] : First
  }>
  : never
: R
...
type X1 = typeof OperatingSystem
type X2 = X1 extends readonly [infer First, ...infer Rest extends string[]] ? First : never
