...
type GetKey<S extends string, Key extends string[] = []> = S extends `${infer First}${Gap}${infer KeyString}${infer Rest}`
  ? GetKey<Rest, [...Key, KeyString]>
  : Key

type X = GetKey<'The result is %%d.'>

type GetControlValues<S extends string[], CK extends string[] = []> = S extends [infer First, ...infer Rest]
? First extends keyof ControlsMap
  ? GetControlValues<Rest, [...CK, ControlsMap[First]]>
  : GetControlValues<Rest, CK>
: CK
type ParsePrintFormat<S extends string> = GetControlValues<GetKey<S>>
