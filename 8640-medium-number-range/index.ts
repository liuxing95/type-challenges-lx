...
type NumToTuple<N extends number, T extends Array<unknown> = []> = 
  T['length'] extends N ?
    T :
    NumToTuple<N, [...T, T['length']]>;

type NumberRange<L extends number, H extends number> = NumToTuple<H> extends [...NumToTuple<L>, ...infer R ]
  ? [...R, H] extends [...infer Res]
    ? Res[number]
    : never
  : never
